<template>
  <div class="app-container">
    ...
    <discountForm v-if="discountInfo.visible" :dialogInfo="discountInfo" @backInfo="init" />
    <!--表格渲染-->
...
export default {
  components: { eForm, eAttr, comForm, killForm, bargainForm, moreForm, discountForm },
  mixins: [initData],
  data() {
    return {
      delLoading: false,
  ...
  methods: {
    ...
    beforeInit() {
      ...
      const sort = 'id,desc'
      this.params = { page: this.page, size: this.size, sort: sort, isShow: 0, isDel: 0 , listContent: this.listContent}
