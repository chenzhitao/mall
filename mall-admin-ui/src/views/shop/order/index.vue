<template>
  ...
  <div class="container">
    ...
    <ePrint ref="form5" :is-add="isAdd" v-if="printInfo.visible" :dialogInfo="printInfo" :print-list="checkList" :to-query="toQuery"/>
    ...
    <el-table ref="multipleTable" v-loading="loading" :data="data" size="small" style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="uid" label="用户昵称" >
        <template slot-scope="scope">
          <span v-if="scope.row.userDTO">{{ scope.row.userDTO.nickname }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="cartInfoList" width="300" label="商品信息">
        <template slot-scope="scope">
            <div style="max-height:150px; "  v-for="(item,index) in scope.row.cartInfoList" :key="index">
                <div    v-if="item.cartInfoMap.productInfo.attrInfo" >
                  <!-- <div>

                  </div> -->
                    <span>
                    <img
                        style="width: 30px;height: 30px;margin:0;cursor: pointer; vertical-align: middle"
    ...
    <el-footer class="footer-contains">
      <div class="footer-search">
        <!-- <el-checkbox v-model="printChecked" @change="batchSelection" style="margin-right: 20px;"></el-checkbox> 这个勾选有问题 -->
  ...
  data() {
    return {
      ...
      queryTypeOptions: [
        ...
        { key: 'realName', display_name: '收货人' },
        { key: 'userPhone', display_name: '收货人电话' },
