<template>
  <el-dialog :append-to-body="true" :close-on-click-modal="false" :before-close="cancel" :visible.sync="dialog" :title="isAdd ? '新增' : '订单详情'" width="700px">
    <el-card>
      ...
      <div class="text item">用户UID:{{form.uid}}</div>
      <div class="text item">用户昵称:{{ form.nickname }}</div>
      <div class="text item">收货人: {{ form.realName }}</div>
      <div class="text item">收货联系电话: {{ form.userPhone }}</div>
      <div class="text item">收货地址: <span v-if="form.storeId>0">{{form.storeAddress}}</span><span v-else>{{form.userAddress }}</span></div>
    </el-card>
    <el-card>
      <div slot="header">
        <span>订单信息</span>
  ...
  methods: {
    ...
    express() {
      let params ={
        ...
        "customerName":this.form.userPhone.substr(7,4)
      }
