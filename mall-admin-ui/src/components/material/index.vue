  ...
  <div v-if="type === 'image'">
    <ul v-for="(item,index) in value" :key="index" class="el-upload-list el-upload-list--picture-card">
      <li tabindex="0" class="el-upload-list__item is-ready" :style="'width: '+width+'px;height: '+height+'px'">
        <div>
          <img :src="item.indexOf('@')>=0?require('@/assets/images/defaultPic.jpg'):item" alt="" class="el-upload-list__item-thumbnail">
...
export default {
  ...
  created() {
    // console.log(this.listDialogVisible,'--------------------------999')
  },
  methods: {
    ...
    uploadFile(content) {
      ...
      upload(content.action, content.file)
        .then((response) => {
          that.handleSuccess(response.data, content.file, [])
        })
    },
    ...
    submitUpload() {
      ...
      //  document.querySelector(".el-upload-list").css('display','none');
